...
public class InstallTask extends SwingWorker<Void, Integer> {
    final String modpackDownloadURL = "https://cdn.bte-germany.de/installer/modpack.zip";
    final String fabricDownloadURL = "https://cdn.bte-germany.de/installer/fabric.zip";
    final String cmdKeybindURL = "https://cdn.modrinth.com/data/h3r1moh7/versions/y3emEjYR/cmdkeybind-1.6.0-1.19.3.jar";
    final String distantHorizonsURL = "https://cdn.modrinth.com/data/uCdwusMi/versions/P4psgCf3/DistantHorizons-1.6.10a-1.19.3.jar";
    final String replayModURL = "https://cdn.modrinth.com/data/Nv2fQJo5/versions/EcNOFu8c/replaymod-1.19.3-2.6.10.jar";
    InstallUtil installUtil;
    JProgressBar progressBar;
    JLabel progessLabel;
    JDialog progressDialog;
    public InstallTask(InstallUtil installUtil, JProgressBar progressBar, JLabel progessLabel, JDialog progressDialog) {
    ...
    protected Void doInBackground() {
        ...
        try {
            ...
            File modpackArchive = new File(installationPath.getAbsolutePath() + "\\modpack.zip");
            modpackArchive.delete();
            downloadOptionalMods(installationPath + "\\mods\\");

            // FABRIC
            downloadFabric(minecraftPath.getAbsolutePath().toString(), new URL(fabricDownloadURL));
            unzipFabric(minecraftPath);
            File fabricArchive = new File(minecraftPath.getAbsolutePath() + "\\versions\\version.zip");
    ...
    private boolean deleteOldFiles(File installationPath) {
        ...
        files.add(new File(installationPath.getAbsolutePath() + "\\config"));
        files.add(new File(installationPath.getAbsolutePath() + "\\fancymenu_data"));
        files.add(new File(installationPath.getAbsolutePath() + "\\fancymenu_setups"));
        files.add(new File(installationPath.getAbsolutePath() + "\\mods"));
        files.add(new File(installationPath.getAbsolutePath() + "\\resources"));
        for (File file : files) {
            if (file.isDirectory()) {
                try {
    ...
    private boolean downloadModpack(String modpackDownloadURL, File installationPath) throws IOException {
        ...
        BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(installationPath.getAbsolutePath() + "\\modpack.zip"), 1024);
    ...
    private boolean unzipModpack(File installationPath) throws IOException {
        ...
        ZipInputStream zis = new ZipInputStream(new FileInputStream(installationPath.getAbsolutePath() + "\\modpack.zip"));
        ZipEntry zipEntry = zis.getNextEntry();
        long fileSize = zipEntry.getCompressedSize();

        while (zipEntry != null) {
            String fileName = zipEntry.getName();
            String filePath = installationPath + "\\" + fileName;
    ...
    private boolean downloadMod(String modsFolder, URL url) throws IOException {
        ...
        BufferedInputStream in = new BufferedInputStream(httpURLConnection.getInputStream());
        BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(modsFolder + "\\" + modName), 1024);
        byte[] buffer = new byte[1024];
        int bytesRead;
        while ((bytesRead = in.read(buffer)) != -1) {
            out.write(buffer, 0, bytesRead);
    ...
    private boolean downloadFabric(String minecraftFolder,URL url) throws IOException{
        ...
        long downloadedFileSize = 0;
        BufferedInputStream in = new BufferedInputStream(httpURLConnection.getInputStream());
        BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(minecraftFolder + "\\versions\\version.zip"), 1024);
        byte[] buffer = new byte[1024];
        int bytesRead;
        while ((bytesRead = in.read(buffer)) != -1) {
            out.write(buffer, 0, bytesRead);
    ...
    private boolean unzipFabric(File minecraftFolder) throws IOException {
        ...
        ZipInputStream zis = new ZipInputStream(new FileInputStream(minecraftFolder + "\\versions\\version.zip"));
        ZipEntry zipEntry = zis.getNextEntry();
        long fileSize = zipEntry.getCompressedSize();

        while (zipEntry != null) {
            String fileName = zipEntry.getName();
            String filePath = minecraftFolder + "\\versions\\" + fileName;
    ...
    private boolean editLauncherProfiles(File minecraftFolder, File installationPath){
        ...
        try {
            FileReader reader = new FileReader(minecraftFolder+"\\launcher_profiles.json");
            Gson gson = new Gson();
            ...
            newEntry.addProperty("lastVersionId", "fabric-loader-0.14.14-1.19.3");
            newEntry.addProperty("name", "BTE Germany v1.0");
            ...
            FileWriter writer = new FileWriter(minecraftFolder+"\\launcher_profiles.json");
